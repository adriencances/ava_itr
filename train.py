...
def train_model(epochs, train_data_size, batch_size, lr=0.01, record=True, do_chkpts=False, chkpt_delay=10, chkpt_file=None):
    ...
    for epoch in range(start_epoch, start_epoch + epochs):
        ...
        if record:
            ...
            writer.add_scalars("training_class_accuracies", dict([(label, train_accs_by_class[i]) for i, label in enumerate(labels)]), global_step=epoch)
            ...
            writer.add_scalars("validation_class_accuracies", dict([(label, val_accs_by_class[i]) for i, label in enumerate(labels)]), global_step=epoch)
